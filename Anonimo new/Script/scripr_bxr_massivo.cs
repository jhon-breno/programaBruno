...
listCaso = [SELECT Id, RecordTypeId, RecordType.DeveloperName, CaseNumber, CNT_Contract__c, CNT_Contract__r.CNT_EndDate__c,CaseRemarks__c,
		...
		FROM Case
		WHERE CaseNumber in :caseNumberList AND Asset.NE__Status__c != 'Disconnected' AND RecordType.DeveloperName = 'CNT_ChangeProduct' and status in ('CNT0001','CNT0002','CNT0009')];
...
for(Case caso : listCaso)
{
	try
	{
			...
			if (caso.PointofDelivery__r.CNT_IsGD__c != null)
			{
				caso.status ='CNT0009';
				ListaCasos.add(caso);
				continue;
...
{
	Database.Update(ListaCasos, false);
}		
		
if(backOfficeList.size()>0)
{
	Database.Insert(backOfficeList, false);
}
