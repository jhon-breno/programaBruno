...
for(Case caso : listCaso)
{
		...
		try
		{
			...
			Boolean synergiaOk = true; // <=== DIEGO SILVA

			if (result != null && result.containsKey('backoffice')) 
			{
				backOfficeList.add((CNT_Contracting_Status_BO__c) result.get('backoffice'));

				for (CNT_Contracting_Status_BO__c bo: backOfficeList) 
				{
					if (bo.Status__c.contains('Sistema Tecnico') && bo.Status__c.contains('ERROR')) 
					{
						synergiaOk = false;
					}
				}
			...
			if (result != null && result.containsKey('workorder')) 
			{
			...
			//if (synergiaOk)
			//{
				if (caso.CNT_LastInvoiceOptions__c == null || caso.CNT_LastInvoiceOptions__c == '1'  || caso.CNT_LastInvoiceOptions__c == '3')
				{
				...
				reqBaja.Body.CreatedDate = date.ValueOf(String.valueOf(caso.createddate));
