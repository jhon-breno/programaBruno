...
listCaso = [select Id, AssetId, PointofDelivery__c, CNT_Potencia__c, CNT_Conexion_Transitoria__c, CNT_Transit_Date_From__c, 
                 ...
                 CNT_Ticket_Expiration_Date__c, Status, CNT_LastInvoiceOptions__c,CreatedDate,CNT_Quote__c,traceerrorinservice__c
...
for(Case caso : listCaso)
{
		...
		try
		{
			...
			if (caso.traceerrorinservice__c != 'Enviado ao Synergia')
			{
				...
				Boolean krlho = (Boolean) result.get('status');
				if (krlho)
					caso.traceerrorinservice__c = 'Enviado ao Synergia';
