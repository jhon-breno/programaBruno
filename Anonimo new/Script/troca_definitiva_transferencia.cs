...
for(String item : caseNumberList)
{
		...
		try
		{
			...
			CNT_LastInvoiceOptions__c,CreatedDate,CNT_Quote__c,CNT_Controller242Success__c
			from Case where CNT_Contract__r.contractnumber = :item.Split('>')[0] AND(RecordType.DeveloperName = 'CNT_OwnershipChange') and status not in ('CNT0008')];
			...
			if (caso.CNT_Controller242Success__c != 'AT')
			{
				...
				{
					if (Integer.ValueOf(caso.casenumber) > 73000000)
						caso.CNT_LastInvoiceOptions__c = '2';				
					else
						caso.CNT_LastInvoiceOptions__c = '7';				
				}
				
				if (Integer.ValueOf(caso.casenumber) < 72000000 || caso.cnt_processStatus__c == 'CNT004')
						caso.CNT_LastInvoiceOptions__c = '7';				
