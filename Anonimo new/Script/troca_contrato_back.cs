...
for(String item : caseNumberList)
{
		...
		try
		{
				...
				Case caso = [select Id, AssetId, PointofDelivery__c, CNT_Potencia__c, CNT_Conexion_Transitoria__c, CNT_Transit_Date_From__c, 
                 ...
                 CNT_Ticket_Expiration_Date__c, Status, CNT_LastInvoiceOptions__c,CreatedDate,CNT_Quote__c,SubCauseBR__c
				from Case where CNT_Contract__r.contractnumber = :item.Split('>')[0] and status != 'CNT0008'];
			
			

			if(caso.SubCauseBR__c == '10')
			{
				reqSyn = CNT_IntegrationDataBR.flow_CT_Tecnica_BR(caso.Id, caso.PointofDeliveryNumber__c);		

			        System.debug('JSON: ' + JSON.serialize(reqSyn));
