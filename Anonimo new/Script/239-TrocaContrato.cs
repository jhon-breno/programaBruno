...
List < String > caseNumberList = new List <String>{'292259726'};
...
listCaso = [select Id, AssetId, PointofDelivery__c, CNT_Potencia__c, CNT_Conexion_Transitoria__c, CNT_Transit_Date_From__c,
    ...
    CNT_Change_Type__c, RecordTypeId, CNT_Economical_Activity__c, Asset.NE__Order_Config__c,Asset.Contract__r.ContractNumber,Asset.Contract__r.CNT_ExternalContract_ID__c,
    ...
    from Case where casenumber IN: caseNumberList and PointofDelivery__r.SegmentType__c = 'B'];
...
for (Case caso: listCaso) 
{
    ...
    try 
    {
        reqAlta = CNT_IntegrationDataBR.flow_CT_Tecnica_BR(caso.Id, caso.Asset.Contract__r.CNT_ExternalContract_ID__c);
		
		//reqAlta = CNT_IntegrationDataBR.flow_CT_Tecnica_BR(caso.Id, caso.PointofDeliveryNumber__c);

        System.debug('JSON: ' + JSON.serialize(reqAlta));

		reqAlta.Body.Submotivo = '10';
		
		if (reqAlta.Body.nome_novo.length() > 40)
			reqAlta.Body.nome_novo = caso.Account.Name.Substring(0,40);			
		
		reqAlta.Body.nome_mae_novo = '';
		
		 reqAlta.Body.complemento = '';
		 
		 reqAlta.Body.nome_logra = '';
		 
		  reqAlta.Body.bairro = '';
		
        HttpResponse response = CNT_IntegrationHelper.CT_239_BRSync(reqAlta, caso.Id);
